...
async function main() {
  ...
  const littleHaitiNFT:LittleHaitiNFT = await LittleHaitiNFT.attach(network.byn);
  console.log("LittleHaitiNFT Address:", littleHaitiNFT.address);

  const txt = await littleHaitiNFT.setDefaultRI("https://bafkreialyaxkvi6tq733bvzttuxpvqgqokzt56w3dkiqt76njq33kgnz34.ipfs.nftstorage.link/");
  console.log("littleHaitiNFT.setBaseURI txt.hash = ",txt.hash);
