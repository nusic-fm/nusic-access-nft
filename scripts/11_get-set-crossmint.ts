...
async function main() {
  ...
  //const txt = await nusicAccessNFT.setCrossmintAddress("0xDa30ee0788276c093e686780C25f6C9431027234");
  const txt = await nusicAccessNFT.setCrossmintAddress(owner.address);
  console.log("nusicAccessNFT.setCrossmintAddress txt.hash = ",txt.hash);
  const txtReceipt = await txt.wait();
  //console.log("txtReceipt = ",txtReceipt); 

  const crossmintAddress = await nusicAccessNFT.crossmintAddress();
  console.log("nusicAccessNFT.crossmintAddress = ",crossmintAddress);
